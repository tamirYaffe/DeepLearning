...
def define_generator(noise_dim, meta):
    ...
    # x = Dense(32, activation="relu", kernel_initializer=init)(input_layer)
    x = Dense(32, kernel_initializer=init)(input_layer)
...
def print_progress(iterations, i, d_real_loss, d_fake_loss, g_loss, batch_size, total_samples):
    ...
    sys.stdout.write("\r%d/%d [%s] - d_real_loss: %f - d_fake_loss: %f - g_loss: %f"
                     % (samples_covered, total_samples, bar, d_real_loss, d_fake_loss, g_loss))
...
def train(data, g_model, d_model, gan_model, noise_dim, epochs, batch_size, early_stop):
    ...
    history = {'d_real_loss': [], 'd_fake_loss': [], 'g_loss': []}
    ...
    for epoch in range(epochs):
        ...
        for i in range(iterations):
            ...
            d_real_loss_tmp, d_fake_loss_tmp = list(), list()
            for _ in range(5):
                ...
                d_real_loss_tmp.append(d_model.train_on_batch(x_real, y_real))
                d_fake_loss_tmp.append(d_model.train_on_batch(x_fake, y_fake))
            d_real_loss = mean(d_real_loss_tmp)
            d_fake_loss = mean(d_fake_loss_tmp)
            ...
            # if (i+1) % 100 == 0:
            history['d_real_loss'].append(d_real_loss)
            history['d_fake_loss'].append(d_fake_loss)
            history['g_loss'].append(g_loss)
            print_progress(iterations, i, d_real_loss, d_fake_loss, g_loss, half_batch_size, len(data))
...
def plot_history(history, y_scale):
    ...
    plt.plot(history['d_real_loss'])
    plt.plot(history['d_fake_loss'])
    plt.plot(history['g_loss'])
    plt.title('Model loss')
    plt.ylabel('Loss')
    plt.xlabel('Iteration')
    plt.legend(['discriminator real loss', 'discriminator fake loss', 'generator loss'], loc='upper right')
    plt.yscale(y_scale)
    plt.show()
...
def part1(action):
    ...
    if action is "train":
        ...
        history = train(transformed_data, generator, discriminator, gan_model, noise_dim,
                        epochs=10, batch_size=128, early_stop=100)
    ...
    if action is "eval":
        ...
        prediction_r = discriminator.predict(x_real)
        print(x_fake)
        print(prediction)
